...
if(empty($_REQUEST)){
	if (!empty($_GET) && !empty($_POST)) $_REQUEST = array_merge($_GET, $_POST);
	elseif (!empty($_GET))$_REQUEST = $_GET;
	elseif (!empty($_POST))$_REQUEST = $_POST;
	else $_REQUEST = array();
}

// check parameter syntax
foreach($vardom as $var => $dom){
	if (!isset($_REQUEST[$var]))$MYREQUEST[$var]=NULL;
	elseif(!is_array($_REQUEST[$var]) && preg_match($dom.'D',$_REQUEST[$var]))$MYREQUEST[$var]=$_REQUEST[$var];
	else $MYREQUEST[$var]=$_REQUEST[$var]=NULL;
}
...
$MY_SELF_WO_SORT=
	...
	"&COUNT=".$MYREQUEST['COUNT'];
$AUTHENTICATED=1;
if ($AUTHENTICATED && isset($MYREQUEST['CC']) && $MYREQUEST['CC'])apcu_clear_cache();
if ($AUTHENTICATED && !empty($MYREQUEST['DU']))apcu_delete($MYREQUEST['DU']);
...
<html>
<head><title>APCu <?php echo $host ?></title>
<style><!--
...
h1.apc div.logo span.logo {background:rgb(119,123,180);color:black;border-right: solid black 1px;border-bottom: solid black 1px;font-style:italic;font-size:1em;padding-left:1.2em;padding-right:1.2em;text-align:right;}
...
div.info {background:rgb(204,204,204);border:solid rgb(204,204,204) 1px;margin-bottom:1em;}
...
div.info table th {background:rgb(204,204,204);margin:0;padding:0.1em 1em 0.1em 1em;}
...
span.box {border: black solid 1px;border-right:solid black 2px;border-bottom:solid black 2px;padding:0 0.5em 0 0.5em;margin-right:1em;}
...
$time=$_SERVER['REQUEST_TIME'];
  echo
		'<div class="info" style="width:860px"><table cellspacing=0><tbody>',
		'<tr>',
		'<th class="left">',sortheader('S',$fieldheading,  "&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader('H','Hits',         "&OB=".$MYREQUEST['OB']),'</th>',
		'<th>',sortheader('Z','Size',         "&OB=".$MYREQUEST['OB']),'</th>',
		'<th class="left">',sortheader('A','Last accessed',"&OB=".$MYREQUEST['OB']),'<br>',date(DATE_FORMAT,$time),'</th>',
		'<th class="left">',sortheader('M','Last modified',"&OB=".$MYREQUEST['OB']),'<br>',date(DATE_FORMAT,$time),'</th>';
...
?>
<script type="text/javascript">setTimeout('window.location.href=window.location.href;',19963);</script>
</body>
</html>
