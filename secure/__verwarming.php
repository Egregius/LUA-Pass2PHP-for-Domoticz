...
if($setpointkamer!=2){
	if($buiten_temp<14 /*&&apcu_fetch('sraamkamer')=='Closed'*/ &&apcu_fetch('sheating')=='On'/*&&(apcu_fetch('traamkamer')<time-7198||time>strtotime('19:00'))*/){
		//$Setkamer=8.0;
...
if($setpointtobi!=2){
	if($buiten_temp<14&&apcu_fetch('sraamtobi')=='Closed'&&$heating=='On'&&(apcu_fetch('traamtobi')<time-7198||time>strtotime('19:10'))){
		//$Settobi=8.0;
		if(date('W')%2==1){
...
if($deurbadkamer=="Open"&&$badkamer_set!=10&&(apcu_fetch('tdeurbadkamer')<time-57||$licht=='Off')){
	ud('badkamer_set',0,10,'badkamer_set 10 deur open');
	$badkamer_set=10.0;
}elseif($deurbadkamer!="Open"&&apcu_fetch('sheating')=='On'){
	if($licht=='On' && apcu_fetch('sbadkamer_set')!=22.0 && ( (time>strtotime('5:00')&&time<strtotime('10:00') )||$timebadkvuur<900)){
		ud('badkamer_set',0,22.0,'badkamer_set 22.0 deur dicht en licht aan');
		$badkamer_set=22.0;
	}elseif($licht=='Off'&&$badkamer_set!=10){
		ud('badkamer_set',0,10,'badkamer_set 10 deur dicht en licht uit');
		$badkamer_set=10.0;
	}
}
