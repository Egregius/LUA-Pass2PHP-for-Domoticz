...
function alles($action,$uit=0){
	...
	if($action=='On'){
		...
		foreach($items as $item)if($s[$item]!='Off'&&strtotime($t[$item])<time-$uit)sw($i[$item],'Off',$item);
		...
		foreach($items as $item)if($s[$item]!='Off')sw($i[$item],'Off',$item);
		$items=array('pirkeuken','pirgarage','pirinkom','pirhall');
		foreach($items as $item)if($s[$item]!='Off')ud($i[$item],0,'Off');
	}
}
function SD($naam){
	global $i;
